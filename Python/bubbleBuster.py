...
lives = 3
alive = True
gim=pygame.image.load("images/GnomeStar.png")
gim=pygame.transform.scale(gim, (110,110))
...
def draw_bubbles():
    for bubble in all_bubbles:
        ...
        screen.blit(gim,(bubble.x -30,bubble.y -20))
...
while True:
    ...
    if alive and score < 3000:
        draw_text('Score: %s' % (score), font, screen, 5, 5)
        draw_text('Lives: %s' % (lives), font, screen, 540, 5)
    if not alive:
        draw_text('Lets Get Right Into the news! Muahahahaha!', font, screen, 100, 5)
        draw_text('Press Enter to Play Again', font, screen, 180, 50)

    if score == 3000:
        draw_text('You defeated GnomeStar!', font, screen, 175, 5)
        draw_text('Press Enter to Play Again', font, screen, 180, 50)
        ball_can_move = False
